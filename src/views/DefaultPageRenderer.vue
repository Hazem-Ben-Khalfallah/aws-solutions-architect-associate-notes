    ...
    <ul class="cards" v-if="pageConfig.tiles">
      ...
      >
        <router-link :to="tile.path">
          <font-awesome-icon class="card__icon" size="2x" :icon="tile.icon" v-if="tile.icon !== 'watson' && tile.icon"/>
          ...
          <i :class="[tile.cssClass]" v-if="tile.cssClass"></i>
          <h4 class="card__title">{{tile.name}}</h4>
          <i v-if="tile.fullName" class="card__subtitle">{{tile.fullName}}</i>
